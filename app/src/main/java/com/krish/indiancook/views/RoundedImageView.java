...
public class RoundedImageView extends ImageView {
    ...
    public RoundedImageView(Context context) {
        this(context, null, R.styleable.RoundedImageViewStyle_RoundedImageViewDefault);
    }

    public RoundedImageView(Context context, AttributeSet attrs) {
        this(context, attrs, R.styleable.RoundedImageViewStyle_RoundedImageViewDefault);
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        hasBorder = attributes.getBoolean(R.styleable.RoundedImageView_civ_border, false);
        hasSelector = attributes.getBoolean(R.styleable.RoundedImageView_civ_selector, false);
        shadowEnabled = attributes.getBoolean(R.styleable.RoundedImageView_civ_shadow, SHADOW_ENABLED);

        // Set border properties, if enabled
        if(hasBorder) {
            int defaultBorderSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setBorderWidth(attributes.getDimensionPixelOffset(R.styleable.RoundedImageView_civ_borderWidth, defaultBorderSize));
            setBorderColor(attributes.getColor(R.styleable.RoundedImageView_civ_borderColor, Color.WHITE));
        }

        // Set selector properties, if enabled
        if(hasSelector) {
            int defaultSelectorSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setSelectorColor(attributes.getColor(R.styleable.RoundedImageView_civ_selectorColor, Color.TRANSPARENT));
            setSelectorStrokeWidth(attributes.getDimensionPixelOffset(R.styleable.RoundedImageView_civ_selectorStrokeWidth, defaultSelectorSize));
            setSelectorStrokeColor(attributes.getColor(R.styleable.RoundedImageView_civ_selectorStrokeColor, Color.BLUE));
        }

        // Set shadow properties, if enabled
        if(shadowEnabled) {
            shadowRadius = attributes.getFloat(R.styleable.RoundedImageView_civ_shadowRadius, SHADOW_RADIUS);
            shadowDx = attributes.getFloat(R.styleable.RoundedImageView_civ_shadowDx, SHADOW_DX);
            shadowDy = attributes.getFloat(R.styleable.RoundedImageView_civ_shadowDy, SHADOW_DY);
            shadowColor = attributes.getColor(R.styleable.RoundedImageView_civ_shadowColor, SHADOW_COLOR);
    ...
    /**
     * Sets the RoundedImageView's border width in pixels.
    ...
    /**
     * Sets the RoundedImageView's basic border color.
     ...
     * RoundedImageView. Be sure to provide some opacity.
    ...
    /**
     * Sets the stroke width to be drawn around the RoundedImageView
    ...
    /**
     * Sets the stroke color to be drawn around the RoundedImageView
     * during click events when the selector is enabled.
    ...
    /**
     * Enables a dark shadow for this RoundedImageView.
